{{-- resources/views/user_guru/read_materi.blade.php --}}
        ...
        <div class="flex-none max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-indigo-200 border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="p-6 pb-0 mb-0 bg-blue border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <h1 class="flex align-items justify-center text-base font-bold leading-7 text-black py-4" style="font-size: 25px;">DETAIL MATERI</h1>
                    <div class="space-y-12">
                        <div class="border-b border-gray-900/10 pb-12">
                            ...
                            <div class="mt-4 sm:col-span-3">
                                ...
                                <div class="mt-2">
                                    @if($materi->file)
                                    <a href="{{ asset('storage/materi/' . $materi->file) }}" target="_blank" class="text-blue-500">{{ $materi->file }}</a>
                                    @else
                    ...
                    <div class="mt-6 flex items-center justify-center gap-x-6">
                        <a href="{{ route('dashboard.guru.materi') }}" class="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all bg-transparent rounded-lg cursor-pointer leading-pro text-xs ease-soft-in shadow-soft-md bg-150 bg-gradient-to-tl from-gray-900 to-slate-800 hover:shadow-soft-xs active:opacity-85 hover:scale-102 tracking-tight-soft bg-x-25">&nbsp;&nbsp;Kembali</a>
                    </div>
                </div>
            </div>
        </div>
