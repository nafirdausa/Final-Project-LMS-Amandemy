        ...
        <div class="flex-none max-w-full px-3">
            <div class="relative flex flex-col min-w-0 mb-6 break-words bg-indigo-200 border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="p-6 pb-0 mb-0 bg-blue border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                    <h1 class="flex align-items justify-center text-base font-bold leading-7 text-black py-4" style="font-size: 25px;">Ujian Siswa</h1>
                    <div class="space-y-12">
                        @foreach($ujian as $u)
                            <div class="border-b border-gray-900/10 pb-12">
                                <h2 class="text-lg font-medium leading-6 text-black">{{ $u->judul }}</h2>
                                <div class="mt-4 p-4 bg-white rounded-lg shadow-md">
                                    <h3 class="text-md font-semibold text-black">Ujian yang Dikumpulkan</h3>
                                    @foreach($u->ujianSiswa as $us)
                                        <div class="mt-4 p-4 bg-gray-100 rounded-lg shadow-inner">
                                            <p class="text-sm text-gray-600">Siswa: {{ $us->siswa->username }}</p>
                                            <a href="{{ asset('storage/ujian_siswa_files/' . $us->file) }}" target="_blank" class="mb-0 text-xs font-semibold leading-tight text-blue-500">{{ $us->file }}</a>
                                            <form action="{{ route('nilai_ujian_siswa', $us->id) }}" method="POST" class="mt-4">
                                                @csrf
                                                <div class="mt-2">
